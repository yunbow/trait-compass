...
### ① セルフチェック(質問に答える)
いくつかの質問に答えると、どの領域の体験を「よくある」と回答したかを、レーダーチャートなどで
見える化します。
...
- **質問への回答・スコアは、スマホやパソコンの中(ブラウザ)だけで処理されます。**
  サーバーに送信されることはありません。
- 「支援窓口を検索する」「AIに説明してもらう」などの機能を使ったときにサーバーへ送られるのは、
  その機能に必要な情報だけです。AI機能は明示的に選んだ場合にのみ動き、送信前に
  「何を送るか・送らないか」を画面上で確認できます(回答そのものは送られません)。
- 通常の閲覧時にも、「どの画面が何回表示されたか」を数えるための小さな通信が発生します。
  Cookieは使わず、記録されるのは「日付 × 画面」ごとの表示回数の合計だけで、回答内容・年齢・
  地域など個人につながる情報は含まれません。ブラウザのトラッキング拒否設定(Do Not Track)が
  有効な場合は、この通信自体を行いません。
- いたずらや過剰な送信を防ぐしくみ(レート制限)では、生のIPアドレスを保存しません。
  IPアドレスから作った、一定時間ごとに変わる復元が難しい符号(SHA-256ハッシュ)だけを
  短時間保存し、期限が過ぎたものは削除します。
...
flowchart TB
  ...
  browser -->|"「支援窓口を探す」「AIに聞く」の利用時に通信<br/>(ほかは個人を識別しない画面到達数の集計のみ)"| server
